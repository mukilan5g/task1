...
class TestCount(unittest.TestCase):
	...
	global str	
	str= 'he is the hero of the movie called *Egypt*!!!,\n and he is born in \t egypt'		#str is a Global variables
	...
	def test_url_responce_pass_not_exists(self):
		self.assertRaises(type(urllib2.URLError), url_responce('http://grepcode.com/repo1.maven.org/maven2/org.apache.bigtop.itest/hadoop-smoke/0.2.0-incubating/examples/text/pg11.txt/?v=source'))
	...
	#CALL the collect_contents function with string that contains special characters and symbols. 
	#CHECK whether it *avoids* those *special characters* and *symbols* .
	def test_collect_contents(self):
		self.assertEqual(['he','is','the','hero','of','the','movie','called','egypt','and','he','is','born','in','egypt'], collect_contents(str.lower()))
	
	#CHECK whether the collect_contents function returns any non word.
	def test_collect_contents_non_word(self):
		self.assertFalse(True in (map(lambda non_word:non_word in [words for words in collect_contents(str.lower())], re.findall(r'\W+',str))))
	
	#PASS list of words and list of stop words in count_frequeny function.
	#CHECK whether the function avoids the stop words and counts the frequency of other words. 
	def test_count_frequeny(self):
		frequency_list = count_frequeny(collect_contents(str.lower()), ['a', 'the', 'he', 'of', 'and', 'in', 'is'])
		self.assertFalse(True in (map(lambda stop_word:stop_word in [words for words,count in frequency_list], ['a', 'the', 'he', 'of', 'and', 'in', 'is'])))
	...
	def test_sort_desc(self):
		frequency_list = count_frequeny(collect_contents(str.lower()), ['a', 'the', 'he', 'of', 'and', 'in', 'is'])
		self.assertEqual([('egypt',2),('hero',1),('born',1),('movie',1),('called',1)],sort_desc(frequency_list))
		self.assertEqual(sorted([('egypt',2),('hero',1),('born',1),('movie',1),('called',1)],reverse=True),sorted(sort_desc(frequency_list),reverse=True))
