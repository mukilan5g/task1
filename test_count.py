...
class TestCount(unittest.TestCase):
	...
	def test_url_responce_pass_not_exists(self):
		self.assertRaises(type(urllib2.URLError), 
						  url_responce('http://grepcode.com/repo1.maven.org/maven2/org.apache.bigtop.itest/hadoop-smoke/0.2.0-incubating/examples/text/pg11.txt/?v=source'))
	...
	def test_url_responce_forbidden(self):
		self.assertRaises(type(urllib2.HTTPError), 
						  url_responce('http://www.gutenberg.org/cache/epub/11/pg11.txt'))
	...
	def test_stop_words_pass_invalid(self):
		self.assertRaises(type(IOError),
						  read_stop_words('C:\Users\mukilan\Desktop\stop_word.txt'))
	...
	def test_collect_contents(self):
		self.assertEqual(['he','is','the','hero','of','the','movie','called','egypt','and','he','is','born','in','egypt'], 
	...
	def test_collect_contents_non_word(self):
		self.assertFalse(True in (map(lambda non_word:non_word in [words for words in collect_contents(str.lower())], 
						 re.findall(r'\W+',str))))
	...
	def test_count_frequeny(self):
		frequency_list = count_frequeny(collect_contents(str.lower()), 
										['a', 'the', 'he', 'of', 'and', 'in', 'is'])
		self.assertFalse(True in (map(lambda stop_word:stop_word in [words for words,count in frequency_list],
									  ['a', 'the', 'he', 'of', 'and', 'in', 'is'])))
	...
	def test_sort_desc(self):
		frequency_list = count_frequeny(collect_contents(str.lower()), 
										['a', 'the', 'he', 'of', 'and', 'in', 'is'])
		self.assertEqual([('egypt',2),('hero',1),('born',1),('movie',1),('called',1)],
						 sort_desc(frequency_list))
		self.assertEqual(sorted([('egypt',2),('hero',1),('born',1),('movie',1),('called',1)],reverse=True),
